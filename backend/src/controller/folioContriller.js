...
export const createFolio = async (req, res) => {
    const {user_id, template_id, main_color, secondry_color, accent_color, font} = req.body;
    const existingFolio = await Folio.findOne({user_id});
    if (existingFolio) {
        return res.status(400).json({message: "Folio already exists for this user"});
    }
    const folio = new Folio({user_id, template_id, main_color, secondry_color, accent_color, font});
    await folio.save();
